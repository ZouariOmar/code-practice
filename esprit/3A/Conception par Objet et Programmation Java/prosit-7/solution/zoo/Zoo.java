...
public class Zoo {
   ...
   * Instruction 32
   *
   * @param animal {@code final Animal}
   * @throws InvalidAgeException
   *
   */
  public void addAnimal(final Animal animal) throws InvalidAgeException {
    if (animal.getAge() < 0)
      throw new InvalidAgeException("Age can't be negative!");

    try {
      if (isZooFull() || animalsLength == ANIMAL_ARRAY_LENGTH)
        throw new ZooFullException("`animals` array reach his limit!");
      if (searchAnimal(animal) != -1)
        throw new ZooFullException("This animal already exists!");
    } catch (final ZooFullException e) {
